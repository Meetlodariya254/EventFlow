    ...
    plugins: [
      react(),
      {
        ...
        configureServer(server) {
          ...
          server.middlewares.use('/api/send-reset-email', (req, res, next) => {
            if (req.method !== 'POST') return next();
            ...
            req.on('end', async () => {
              try {
                ...
                const smtpPass = env.SMTP_PASS || env.VITE_SMTP_PASS || env.EMAIL_PASS;
                const smtpHost = env.SMTP_HOST || env.VITE_SMTP_HOST || 'smtp.gmail.com';
                ...
                if (smtpUser && smtpPass) {
                  ...
                  console.log('  ⚠️ No SMTP credentials in .env. Using automatic Ethereal test server...');
